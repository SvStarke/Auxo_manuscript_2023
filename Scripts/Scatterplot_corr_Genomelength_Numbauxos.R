...
length_count <- ggplot(Auxotrophy_12, aes(x =`Genome Length (bp)`, y= count)) +
  ...
  theme_minimal()+
  theme(axis.title.x = element_text(color = "black", size = 10, margin = margin(t = 10, r = 0, b= 0, l = 0))) +
  theme(axis.title.y = element_text(colour = "black", size = 10,  margin = margin(t =0, r = 10, b= 0, l = 0))) +
